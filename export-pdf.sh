...
#   - Node.js >= 18
#   - pnpm
#   - md-to-pdf (se descarga automaticamente con pnpm dlx)
# =============================================================================
...
# Validaciones
if [[ ! -d "$WEEK_DIR" ]]; then
  echo -e "${RED}Error: No se encontro la carpeta '$WEEK_DIR'${NC}"
...
if ! command -v pnpm &>/dev/null; then
  echo -e "${RED}Error: pnpm no esta instalado${NC}"
  echo "   Instalalo con: npm install -g pnpm"
  exit 1
fi

# Detectar navegador Chromium
CHROMIUM_EXEC=""
for candidate in \
...
if [[ -z "$CHROMIUM_EXEC" ]]; then
  echo -e "${RED}Error: No se encontro ningun navegador Chromium (brave, chromium, etc.)${NC}"
  exit 1
fi
...
echo -e "${BOLD}${BLUE}==============================================${NC}"
echo -e "${BOLD}${BLUE}  Exportador PDF - Bootcamp GitLab CE${NC}"
echo -e "${BOLD}${BLUE}==============================================${NC}"
...
if [[ ${#MD_FILES[@]} -eq 0 ]]; then
  echo -e "${YELLOW}No se encontraron archivos .md en $WEEK${NC}"
  exit 0
fi
...
# Descargar md-to-pdf
echo -e "${YELLOW}Verificando md-to-pdf (puede tardar en la primera ejecucion)...${NC}"
...
if [[ "$MERGE" == "--merge" ]]; then
  ...
  TEMP_MERGED="$(mktemp /tmp/bootcamp-gitlab-XXXXXX.md)"
  trap 'rm -f "$TEMP_MERGED"' EXIT

  {
    echo "---"
    echo "title: Bootcamp GitLab CE - $(echo "$WEEK" | sed 's/-/ /g' | tr '[:lower:]' '[:upper:]')"
    echo "date: $(date '+%B %Y')"
    echo "---"
    echo ""
  } > "$TEMP_MERGED"
  ...
  if pnpm dlx md-to-pdf \
    ...
    echo -e "  PDF combinado : ${GREEN}OK ${MERGED_PDF/$SCRIPT_DIR\//}${NC}"
  else
    echo -e "  PDF combinado : ${RED}fallo${NC}"
  fi
  echo ""
fi
